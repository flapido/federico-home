  ...
  return (
    <div>
      ...
      <section id="contacto" className="scroll-mt-20 mx-auto max-w-[1280px] px-5 py-14 sm:px-6 md:px-8 md:py-20">
        <div className="rounded-[22px] bg-ink p-7 text-paper md:flex md:items-center md:justify-between md:gap-8 md:p-10"><div><div className="font-mono text-[10px] uppercase tracking-[.16em] text-brass">Contacto</div><h2 className="mt-3 font-display text-[32px] leading-none">¿Tenés una idea o una consulta?</h2><p className="mt-3 max-w-[58ch] text-[13px] leading-relaxed text-paper/70">No hace falta tener todo definido. Podemos conversar sobre una mejora, una demo o una primera versión.</p></div><div className="mt-6 flex flex-wrap gap-2 md:mt-0 md:shrink-0"><Link to="/contacto" className="rounded-full bg-white px-4 py-2.5 text-[13px] text-ink">Contame tu idea →</Link><a href="https://wa.me/5491157642626?text=Hola%20Federico%2C%20vi%20tu%20portfolio%20y%20quer%C3%ADa%20hacerte%20una%20consulta." target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/20 px-4 py-2.5 text-[13px] text-paper">WhatsApp ↗</a></div></div>
      </section>
    </div>
  )
}
