...
export const onRequestPost = async ({ request, env }: Context) => {
  ...
  const now = unixNow();
  const active = await env.ANALYTICS_DB.prepare("SELECT id, code_hash, attempts, expires_at FROM console_otp_codes WHERE used_at IS NULL ORDER BY created_at DESC LIMIT 1").all<{ id: number; code_hash: string; attempts: number; expires_at: number }>();
  const otp = active.results[0];

  if (!otp || otp.expires_at <= now || otp.attempts >= CONSOLE_OTP_ATTEMPT_LIMIT) {
    if (otp) await env.ANALYTICS_DB.prepare("UPDATE console_otp_codes SET used_at = ? WHERE id = ?").bind(now, otp.id).run();
    return denied();
  }
  ...
  if (!constantTimeEqual(hash, otp.code_hash)) {
    ...
    await env.ANALYTICS_DB.prepare("UPDATE console_otp_codes SET attempts = ?, used_at = CASE WHEN ? >= ? THEN ? ELSE used_at END WHERE id = ? AND used_at IS NULL").bind(attempts, attempts, CONSOLE_OTP_ATTEMPT_LIMIT, now, otp.id).run();
    return denied();
  }

  const consumed = await env.ANALYTICS_DB.prepare("UPDATE console_otp_codes SET used_at = ? WHERE id = ? AND used_at IS NULL AND expires_at > ? AND attempts < ?").bind(now, otp.id, now, CONSOLE_OTP_ATTEMPT_LIMIT).run();
